...
beforeEach( async () => {
    ...
    await factory.methods.createDonation(accounts[2], web3.utils.toWei('5', 'ether'), 'Goat')
...
describe( "Factory", ()=> {
  ...
  it('You can not send money to yourself', async () => {
    try{
      await factory.methods.createDonation(accounts[2], '1000', 'Bicycle')
...
describe('Donation', () => {
    ...
    it('allow all users to donate', async () => {
      await donation.methods.donate(0)
            .send({from: accounts[0], value: '10'});

      await donation.methods.donate(0)
            .send({from: accounts[0], value: '20'});

            await donation.methods.donate(0)
            .send({from: accounts[1], value: '5'});
      await donation.methods.donate(0)
            .send({from: accounts[1], value: '30'});

      await donation.methods.donate(0)
            .send({from: accounts[2], value: '10'});

      await donation.methods.donate(0)
            .send({from: accounts[3], value: '10'});

      await donation.methods.donate(0)
            .send({from: accounts[4], value: '5'});
      ...
      const contribution0 = await donation.methods.getContribution(accounts[0]).call();
      const contribution1 = await donation.methods.getContribution(accounts[1]).call();
    ...
    it('changes the status', async () => {
      ...
      await donation.methods.donate(0).send({from: accounts[0], value: web3.utils.toWei('5', 'ether')});
    ...
    it('fails when other persons try to complete', async () => {
      ...
      try {
        await donation.methods.complete(0)
    ...
    it('works the whole flow', async () => {
        ...
        await donation.methods.donate(0).send({from: accounts[0], value: web3.utils.toWei('3', 'ether')});
        assert.equal('pending', await donation.methods.status().call());
        await donation.methods.donate(0).send({from: accounts[1], value: web3.utils.toWei('2', 'ether')});
        ...
        assert.equal(web3.utils.toWei('3', 'ether'), await donation.methods.getContribution(accounts[0]).call())
        assert.equal(web3.utils.toWei('2', 'ether'), await donation.methods.getContribution(accounts[1]).call())
